			...
			if (evaluateEvent(event, scheduleConfigEvent)) {
				...
				if (action.equalsIgnoreCase(ActionType.enroll.toString())) {
					...
					if (scheduleName.equalsIgnoreCase(vitaScheduleName)) {
						...
						if (enrollment != null) {
							scheduler.unEnrollFromSchedule(event.getBaseEntityId(), event.getProviderId(), vitaScheduleName,
							    event.getId());
						}
					}
					...
					if (scheduleName.equalsIgnoreCase("MEASLES 1")) {
						scheduler
						        .unEnrollFromSchedule(event.getBaseEntityId(), event.getProviderId(), "MR 1", event.getId());
						scheduler
						        .unEnrollFromSchedule(event.getBaseEntityId(), event.getProviderId(), "MR 2", event.getId());
					} else if (scheduleName.equalsIgnoreCase("MR 1")) {
						scheduler.unEnrollFromSchedule(event.getBaseEntityId(), event.getProviderId(), "MEASLES 1",
						    event.getId());
						scheduler.unEnrollFromSchedule(event.getBaseEntityId(), event.getProviderId(), "MEASLES 2",
						    event.getId());
					}
