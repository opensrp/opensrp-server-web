...
public class ChildScheduleHandler extends BaseScheduleHandler {
				...
				if (action.equalsIgnoreCase(ActionType.enroll.toString())) {
					...
					if (children != null && !children.isEmpty()) {
						for (Client client : children) {
							enccSchedulesService.enrollIntoCorrectMilestoneOfENCCCare(client.getBaseEntityId(),
							    scheduleName,
							    LocalDate.parse(getReferenceDateForSchedule(event, scheduleConfigEvent, action)),
							    event.getId());
						}
					}
				} else if (action.equalsIgnoreCase(ActionType.fulfill.toString())) {
					enccSchedulesService.fullfillMilestone(event.getBaseEntityId(), event.getProviderId(), scheduleName,
	...
	/**
	 * Currently this handler is using the birthnotification event for the first milestone since
	 * it's the one that has the date of birth/date of outcome concept. This event has the mother's
	 * baseentityid but we are interested in the child's/chidlrens' baseentityid However if the dob
	 * can also be added to the ENCC event, that would be great! use nbnf event baseentityid as the
	 * mother's id and the event date created this method adds and substracts 1 so that the date is
	 * in a range just in case a kid was born at 11:59 pm
	 * 
	 * @param event
	 * @return
	 */
