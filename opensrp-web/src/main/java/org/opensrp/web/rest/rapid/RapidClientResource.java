...
public class RapidClientResource {
	...
	@Autowired
	public RapidClientResource(ClientService clientService, EventService eventService, AllFormSubmissions allFormSubmission,
	    FormEntityConverter fec, PatientService ps, EncounterService es) {
	...
	public Map<String, Object> createChild(HttpServletRequest req) {
		...
		if (StringUtils.isEmptyOrWhitespaceOnly(identifier) || StringUtils.isEmptyOrWhitespaceOnly(firstName)
		        || StringUtils.isEmptyOrWhitespaceOnly(lastName) || StringUtils.isEmptyOrWhitespaceOnly(gender)
		        || StringUtils.isEmptyOrWhitespaceOnly(birthdate)) {
			res.put("ERROR",
			    "Request MUST have parameters identifier, firstName, lastName, gender, birthdate with valid values");
			return res;
		}
	...
	private static Map<String, Object> makeVaccineCard(List<Event> el, List<String> eligibleVaccines) {
		...
		for (Event e : el) {
			...
			for (Obs o : e.getObs()) {
				...
				if (o.getFormSubmissionField().toString().toLowerCase()
				        .matches("bcg|penta1|penta2|penta3|measles1|measles2")) {
	...
	public Map<String, String> updateVaccineO(HttpServletRequest req) {
		...
		try {
			...
			Event e = new Event(c.getBaseEntityId(), "Immunization", new DateTime(), "testentity", "demotest", location,
	...
	public Map<String, String> updateVaccine(HttpServletRequest req) {
		...
		try {
			...
			FormData form = new FormData(bindType, "/model/instance/Child_Vaccination_Followup/", generateChildFields(eid,
			    insId, location, user, location, DateTime.now(), vaccine, new DateTime(dt)), null);
			FormInstance formInstance = new FormInstance(form, "1");
			FormSubmission fs = new FormSubmission(user, insId, "child_followup", eid, DateTime.now().getMillis(), "1",
			        formInstance, DateTime.now().getMillis());
	...
	private List<FormField> generateChildFields(String entityId, String instanceId, String uc, String provider,
	                                            String center, DateTime eDate, String vaccine, DateTime vaccineDate) {
