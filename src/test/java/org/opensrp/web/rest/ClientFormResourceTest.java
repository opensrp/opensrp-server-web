...
public class ClientFormResourceTest {
	...
	public void testSearchForFormByFormVersionShouldReturnSpecificJsonFormVersion() throws Exception {
		...
		when(clientFormService.getClientFormMetadataByIdentifierAndVersion(formIdentifier, formVersion, false))
				.thenReturn(clientFormMetadata);
	...
	public void testSearchForFormByFormVersionShouldReturnSpecificJsonValidatorVersion() throws Exception {
		...
		when(clientFormService.getClientFormMetadataByIdentifierAndVersion(formIdentifier, formVersion, true))
				.thenReturn(clientFormMetadata);
		when(clientFormService.getClientFormById(3L)).thenReturn(clientForm);
		when(clientFormService.getClientFormMetadataById(3L)).thenReturn(clientFormMetadata);
	...
	@Test
	public void testSearchForFormByFormVersionShouldReturnNoContentWhenVersionHasntChangedAndStrictIsTrue()
			throws Exception {
		String formIdentifier = "opd/reg.json";
	...
	@Test
	public void testSearchForFormByFormVersionShouldReturn404WhenStrictIsTrueAndCurrentFormVersionIsOutOfDate()
			throws Exception {
		String formIdentifier = "opd/reg.json";
		String formVersion = "0.1.1";
	...
	public void testSearchForFormByFormVersionShouldReturnNextJsonValidatorVersion() throws Exception {
		...
		when(clientFormService.getClientFormMetadataByIdentifierAndVersion(formIdentifier, formVersion, true))
				.thenReturn(null);
		when(clientFormService.getAvailableClientFormMetadataVersionByIdentifier(formIdentifier, true))
				.thenReturn(idVersionTuples);
		when(clientFormService.getClientFormById(3L)).thenReturn(clientForm);
		when(clientFormService.getClientFormMetadataById(3L)).thenReturn(clientFormMetadata);
	...
	public void testAddClientFormWhenGivenJSON() throws Exception {
		...
		when(clientFormService.addClientForm(any(ClientForm.class), any(ClientFormMetadata.class)))
				.thenReturn(mock(ClientFormService.CompleteClientForm.class));
		...
		ArgumentCaptor<ClientFormMetadata> clientFormMetadataArgumentCaptor = ArgumentCaptor
				.forClass(ClientFormMetadata.class);
		verify(clientFormService)
				.addClientForm(clientFormArgumentCaptor.capture(), clientFormMetadataArgumentCaptor.capture());
	...
	public void testAddClientFormWhenGivenJSONWithMissingFieldsShouldReturn400() throws Exception {
		...
		formValidator.setJson(
				"\"{\"cannot_remove\":{\"title\":\"Fields you cannot remove\",\"fields\":[\"reaction_vaccine_duration\",\"reaction_vaccine_dosage\",\"aefi_form\"]}}\"");
		...
		assertThat(response, StringStartsWith
				.startsWith("Form upload failed.Kindly make sure that the following fields are still in the form : "));
	...
	public void testAddClientFormWhenGivenRulesYaml() throws Exception {
		...
		assertEquals(TestFileContent.CALCULATION_YAML_FILE_CONTENT,
				clientFormArgumentCaptor.getValue().getJson().toString());
		ClientFormMetadata clientFormMetadata = clientFormMetadataArgumentCaptor.getValue();
		assertEquals(formIdentifier, clientFormMetadata.getIdentifier());
		assertEquals("0.0.2", clientFormMetadata.getVersion());
		assertEquals(formName, clientFormMetadata.getLabel());
	...
	public void testCheckValidContentShouldReturnErrorMessageWhenGivenInvalidJSONStructure() {
		ClientFormResource clientFormResource = webApplicationContext.getBean(ClientFormResource.class);
		assertNotNull(clientFormResource
				.checkValidJsonYamlPropertiesStructure(TestFileContent.JSON_FORM_FILE.substring(0, 10), "application/json"));
	}
	...
	public void testCheckValidJsonYamlPropertiesStructureShouldReturnErrorMessageWhenGivenInvalidYamlStructure() {
		ClientFormResource clientFormResource = webApplicationContext.getBean(ClientFormResource.class);
		assertNotNull(clientFormResource
				.checkValidJsonYamlPropertiesStructure(TestFileContent.INVALID_YAML_FILE_CONTENT, "application/x-yaml"));
	}

	@Test
	public void testCheckValidJsonYamlPropertiesStructureShouldReturnErrorMessageWhenGivenInvalidPropertiesStructure() {
		ClientFormResource clientFormResource = webApplicationContext.getBean(ClientFormResource.class);
		assertNotNull(clientFormResource.checkValidJsonYamlPropertiesStructure(
				TestFileContent.JMAG_PROPERTIES_FILE_CONTENT.substring(0, 378) + "\\uxxxx",
