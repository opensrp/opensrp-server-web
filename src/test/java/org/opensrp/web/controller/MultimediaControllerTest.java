...
public class MultimediaControllerTest {
	...
	public void testUploadShouldUploadFileWithCorrectName() throws Exception {
		...
		// verify call
		verify(multimediaService)
				.saveFile(any(MultimediaDTO.class), any(byte[].class), stringArgumentCaptor.capture());
	...
	public void testDownloadWithAuth() {
		...
		doReturn(getMockedAuthentication()).when(provider).authenticate(any(Authentication.class));
	...
	public void testUploadFilesWithMimeTypeThatIsNotAllowedReturnsBadRequest() throws Exception {
		MultipartFile multipartFile = mock(MultipartFile.class);
		doReturn("originalName").when(multipartFile).getOriginalFilename();
		doReturn("pdf").when(multipartFile).getContentType();
		doReturn(new byte[10]).when(multipartFile).getBytes();
